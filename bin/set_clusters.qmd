...
cat("### RNA markers\n")
lapply(params$markers_rna[c(1:3)], function(x){
  p <- clustree(seurat, prefix = "wsnn_res.", node_colour = x, node_colour_aggr = "mean") + 
          scale_colour_viridis_c(option = "plasma") +
          ggtitle(x) +
          theme(legend.position = "bottom", legend.direction = "horizontal") 
  p <- clustree_arrow(p, selected_res = params$clustering2_res)
  print(p)
})
...
lapply(params$markers_adt, function(x){
  p <- clustree(seurat, prefix = "wsnn_res.", node_colour = x, node_colour_aggr = "mean") + 
          scale_colour_viridis_c(option = "plasma") +
          ggtitle(x) +
          theme(legend.position = "bottom", legend.direction = "horizontal") 
...
cat("### RNA markers\n")
DefaultAssay(seurat) <- "RNA"
lapply(params$markers_rna, function(x){
  p <- clustree(seurat, prefix = "RNA_snn_res.", node_colour = x, node_colour_aggr = "mean") + 
          scale_colour_viridis_c(option = "plasma") +
          ggtitle(x) +
          theme(legend.position = "bottom", legend.direction = "horizontal") 
  p <- clustree_arrow(p, selected_res = params$clustering2_res)
...
cat("### ATAC Gene Activity markers\n")
DefaultAssay(seurat) <- "RNA"
lapply(params$markers_rna, function(x){
  p <- clustree(seurat, prefix = "ATAC_snn_res.", node_colour = x, node_colour_aggr = "mean") + 
          scale_colour_viridis_c(option = "plasma") +
          ggtitle(x) +
          theme(legend.position = "bottom", legend.direction = "horizontal") 
  p <- clustree_arrow(p, selected_res = params$clustering2_res)
...
if(params$rna_normalization_method != "sct"){
  seurat <- ScaleData(seurat, features = unique(c(VariableFeatures(seurat),top_rna_markers$gene)), vars.to.regress = latent_vars)
}


p <- DoHeatmap(seurat, features = unique(top_rna_markers$gene), raster = T) +
    theme(legend.position = "bottom", legend.direction = "horizontal")
...
p <- DoHeatmap(seurat, features = unique(top_adt_markers$gene), raster = T) 
    theme(legend.position = "bottom", legend.direction = "horizontal")
